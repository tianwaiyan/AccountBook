...
export class DemoAnalyticsRepository implements AnalyticsRepository {
  async monthSummary(bookId: string, yearMonth: string): Promise<MonthSummary> { const rows = personalRows(bookId).filter((row) => row.occurredAt.startsWith(yearMonth)); const incomeMinor = sum(rows.filter((row) => row.tradeType === "income").map((row) => Math.abs(row.amountMinor))); const expenseMinor = sum(rows.map((row) => row.tradeType === "expense" ? Math.abs(row.amountMinor) : row.tradeType === "refund" ? -Math.abs(row.amountMinor) : 0)); const all = transactions.filter((row) => row.bookId === bookId); return { incomeMinor, expenseMinor, balanceMinor: incomeMinor - expenseMinor, count: rows.length, passThroughOutgoingMinor: sum(all.filter((row) => row.categorySystemKey === "pass_through_expense").map(signedSpecialAmount)), passThroughIncomingMinor: sum(all.filter((row) => row.categorySystemKey === "pass_through_income").map(signedSpecialAmount)), pendingReimbursementMinor: sum(all.filter((row) => row.categorySystemKey === "public_expense" && row.statusCode === "pending_reimbursement").map(signedSpecialAmount)), settledReimbursementMinor: sum(all.filter((row) => row.categorySystemKey === "public_expense" && row.statusCode === "settled").map(signedSpecialAmount)) }; }
...
function signedSpecialAmount(row: Transaction): number { return row.tradeType === "refund" ? -Math.abs(row.amountMinor) : Math.abs(row.amountMinor); }
function tracking(bookId: string, status: Transaction["statusCode"]): TrackingRecord[] { const systemKey = status === "pending_reimbursement" ? "public_expense" : status === "pending_transfer" ? "pass_through_income" : null; return transactions.filter((row) => row.bookId === bookId && row.statusCode === status && (!systemKey || row.categorySystemKey === systemKey)).map((row) => ({ id: row.id, occurredAt: row.occurredAt, counterparty: row.counterparty, remark: row.remark, amountMinor: Math.abs(row.amountMinor), statusCode: row.statusCode })); }

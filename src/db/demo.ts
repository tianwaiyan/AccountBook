...
export class DemoAnalyticsRepository implements AnalyticsRepository {
  async monthSummary(bookId: string, yearMonth: string): Promise<MonthSummary> { const rows = personalRows(bookId).filter((row) => row.occurredAt.startsWith(yearMonth)); const incomeMinor = sum(rows.filter((row) => row.tradeType === "income").map((row) => Math.abs(row.amountMinor))); const expenseMinor = sum(rows.map((row) => row.tradeType === "expense" ? Math.abs(row.amountMinor) : row.tradeType === "refund" ? -Math.abs(row.amountMinor) : 0)); const all = transactions.filter((row) => row.bookId === bookId); return { incomeMinor, expenseMinor, balanceMinor: incomeMinor - expenseMinor, count: rows.length, passThroughOutgoingMinor: sum(all.filter((row) => row.occurredAt.startsWith(yearMonth) && row.categorySystemKey === "pass_through_expense").map((row) => Math.abs(row.amountMinor))), passThroughIncomingMinor: sum(all.filter((row) => row.occurredAt.startsWith(yearMonth) && row.categorySystemKey === "pass_through_income").map((row) => Math.abs(row.amountMinor))), pendingReimbursementMinor: sum(all.filter((row) => row.statusCode === "pending_reimbursement").map((row) => Math.abs(row.amountMinor))), settledReimbursementMinor: sum(all.filter((row) => row.categorySystemKey === "public_expense" && row.statusCode === "settled").map((row) => Math.abs(row.amountMinor))) }; }

...
import { matchesKeyword, parseKeywordExpression } from "@/utils/transaction-search";

describe("transaction keyword expressions", () => {
  it("gives AND higher priority than OR", () => {
    expect(parseKeywordExpression("水果 AND 超市 OR 咖啡")).toEqual([["水果", "超市"], ["咖啡"]]);
  });

  it("supports Chinese operators and case-insensitive matching", () => {
    ...
    expect(matchesKeyword(["Coffee Shop"], "coffee")).toBe(true);
  });
});

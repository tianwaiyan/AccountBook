...
export function ImportPage({ referenceData, onChanged }: { referenceData: ReferenceData; onChanged: () => void }) {
  ...
  return <div className="space-y-5">
    ...
    {preview && <Card><CardHeader><CardTitle>导入预览</CardTitle><div className="flex gap-2"><Badge>{preview.candidates.length} 条候选</Badge><Badge tone={preview.excluded.length ? "warning" : "neutral"}>{preview.excluded.length} 条过滤</Badge></div></CardHeader><CardContent className="space-y-4"><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{mappingGroups.map((candidate) => <div key={`${candidate.tradeType}:${candidate.sourceCategory}`} className="rounded-md border border-border p-2"><p className="truncate text-xs text-muted-foreground">{tradeTypeLabels[candidate.tradeType]} · {candidate.sourceCategory || "空白分类"}</p><select className="mt-1 h-9 w-full rounded-md border border-input bg-background px-2 text-sm" value={candidate.categoryId ?? ""} onChange={(event) => setMapping(candidate, event.target.value)}><option value="">待分类</option>{categoriesFor(candidate.tradeType, referenceData).map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</select></div>)}</div><div className="max-h-80 overflow-auto rounded-md border border-border"><table className="w-full min-w-[760px] text-xs"><thead className="sticky top-0 bg-muted"><tr><th className="px-3 py-2 text-left">时间</th><th className="px-3 py-2 text-left">账户</th><th className="px-3 py-2 text-left">收支</th><th className="px-3 py-2 text-right">金额</th><th className="px-3 py-2 text-left">来源分类</th><th className="px-3 py-2 text-left">备注</th></tr></thead><tbody>{preview.candidates.slice(0, 100).map((candidate) => <tr key={candidate.rowId} className="border-t border-border"><td className="px-3 py-2">{candidate.occurredAt}</td><td className="px-3 py-2">{candidate.accountName}</td><td className="px-3 py-2">{tradeTypeLabels[candidate.tradeType]}</td><td className="px-3 py-2 text-right">{formatMoney(candidate.amountMinor)}</td><td className="px-3 py-2">{candidate.sourceCategory}</td><td className="max-w-60 truncate px-3 py-2">{candidate.remark}</td></tr>)}</tbody></table></div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setPreview(null)}>取消</Button><Button onClick={commit} disabled={busy}><Upload className="size-4" />导入 {preview.candidates.length} 条</Button></div></CardContent></Card>}

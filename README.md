...
GitHub Release 提供 `AccountBook-v2.0.0-windows-x64.zip`。请先将压缩包完整解压到普通且具有写入权限的文件夹，例如 `D:\AccountBook`，再双击其中的 `AccountBook\AccountBook.exe`。
...
开发环境继续使用 Vite 和 Tauri 开发模式。便携发布构建命令为：
...
└─ AccountBook-v2.0.0-windows-x64.zip
```

GitHub Release 只上传运行 ZIP；GitHub 会自动提供源码归档，不需要另行上传源码 ZIP，也不上传单独的裸 EXE。
